...
class AZURE():
    ...
    def azure_connect(self, account_name=None, account_key=None):
        ...
        try:
            ...
            logger.info(f"Making azure service connection to {account_name}")
            self.file_service = FileService(account_name, account_key)
            logger.info(f"Connection established for Azure service")

        except Exception as e:
            logger.warning(
                f'Something went wrong while connecting to Azure(azure_connect). Check Logs...')
            logger.warning('Error while connecting: {}'.format(str(e)))
            self.status = False
            return False

    def insert_file_azure(
            self,
            share_name,
            directory_name,
            file_name,
        ...
        try:
            self.file_service.create_file_from_bytes(
                share_name, directory_name, file_name, file_data)
            return True
        except Exception as e:
            logger.warning(
                f'Something went wrong while connecting to Azure(insert_file_azure). Check Logs...')
    ...
    def create_directory_azure(self, share_name, directory_name):
        ...
        try:
            ...
            if status is False:
                status = self.file_service.create_directory(
                    share_name, directory_name)
            return status
        except Exception as e:
            logger.warning(
                f'Something went wrong while connecting to Azure(create_directory_azure). Check Logs...')
            logger.warning('Error: {}'.format(str(e)))
            self.status = False
            return False
    ...
    def list_files_and_dir_azure(self, share_name, dir_name):
        ...
        try:
            generator = self.file_service.list_directories_and_files(
                share_name=share_name, directory_name=dir_name)
            dir_list = []
            ...
            logger.warning(
                f'Something went wrong while connecting to Azure(list_files_and_dir_azure). Check Logs...')
            logger.warning('Error: {}'.format(str(e)))
            self.status = False
            return False

    def download_file_azure(
            self,
            share_name,
            directory_name,
            file_name,
        ...
        try:
            ...
            self.file_service.get_file_to_path(
                share_name, directory_name, file_name, destination_file)
            return {"status": True}

        except Exception as e:
            logger.warning(
                f'Something went wrong while connecting to Azure(download_file_azure). Check Logs...')
            logger.warning('Error: {}'.format(str(e)))
            self.status = False
            return False
    ...
    def file_url_in_azure(self, share_name, directory_name, file_name):
        ...
        """
        try:
            sourcefile = self.file_service.make_file_url(
                share_name, directory_name, file_name)
            return {"sourcefile": sourcefile}
        except Exception as e:
            logger.warning(
                f'Something went wrong while connecting to Azure(file_url_in_azure). Check Logs...')
            logger.warning('Error: {}'.format(str(e)))
            self.status = False
            return False
    ...
    def copy_file_azure(
        ...
        """
        try:
            copy = self.file_service.copy_file(
                share_name, directory_name, file_name, copy_source)
            if(copy.status == 'pending'):
                self.file_service.abort_copy_file(
                    share_name, None, 'file1copy', copy.id)
            return {"status": True}
        except Exception as e:
            logger.warning(
                f'Something went wrong while connecting to Azure(copy_file_azure). Check Logs...')
            logger.warning('Error: {}'.format(str(e)))
            self.status = False
            return False
    ...
    def delete_file_azure(self, share_name, directory_name, file_name):
        ...
        try:
            ...
            logger.warning(
                f'Something went wrong while connecting to Azure(delete_file_azure). Check Logs...')
            logger.warning('Error: {}'.format(str(e)))
            self.status = False
            return False

    def move_file_in_azure(
            self,
            share_name,
        ...
        """
        try:
            result = self.file_url_in_azure(
                share_name, source_directory_name, file_name)
            if "message" in result:
                return result

            """ Checking file path exist or not """
            file_status = self.file_service.exists(
                share_name, destination_directory_name + '/')
            ...
            logger.warning(
                f'Something went wrong while connecting to Azure(move_file_in_azure). Check Logs...')
            logger.warning('Error: {}'.format(str(e)))
            self.status = False
            return False

    def copy_file_in_azure(
            self,
            share_name,
            source_directory_name,
            destination_directory_name,
        ...
        try:
            if old_file_name is not None:
                result = self.file_url_in_azure(
                    share_name, source_directory_name, old_file_name)
            else:
                result = self.file_url_in_azure(
                    share_name, source_directory_name, file_name)
            ...
            logger.warning(
                f'Something went wrong while connecting to Azure(copy_file_in_azure). Check Logs...')
            logger.warning('Error: {}'.format(str(e)))
            self.status = False
            return False
    ...
    def path_file_generator(self, generate_path):
        ...
        """
        try:
            for file in self.file_service.list_directories_and_files(
                    generate_path):
        ...
        try:
            ...
            if 'Planner' in userinfo_obj:
                if userinfo_obj['Planner']:
                    try:
                        userinfo_obj['Planner'] = userinfo_obj['Planner'][:userinfo_obj['Planner'].index(
                            '@')].replace('.', ' ')  # if planner is an emali
                    except BaseException:
                        pass
                else:
                    userinfo_obj['Planner'] = 'Sample_User'
            if 'Brand' in userinfo_obj:
                userinfo_obj['Brand'] = userinfo_obj['Brand'].replace(
                    '/', ' or')
            for placeholder in userinfo_obj:
                if path_format.find(placeholder) != -1:
                    path_format = path_format.replace(
                        str(placeholder), str(userinfo_obj[placeholder]))
                else:
                    pass
            ...
            logger.warning(
                f'Something went wrong while connecting to Azure(path_conf_helper). Check Logs...')
            logger.warning('Error while connecting: {}'.format(str(e)))
            self.status = False
            return False
